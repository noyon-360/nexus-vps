    ...
    conn.on("ready", () => {
      ...
      conn.exec(cmd, (err, stream) => {
        ...
        stream.on("close", () => {
          ...
          try {
            const sections = output.split(/---[A-Z0-9]+---/); // Updated regex to match ---PM2---
            ...
            // Format: /path/to/file:   server_name example.com www.example.com;
            const domainMap = new Map<string, Set<string>>();

            domainLines.forEach(line => {
                if (!line || line === "No sites found") return;
                
                // Split by first colon to get file path and content
            ...
            try {
                // Ensure we only parse the JSON part if there's extra noise
                const start = pm2Json.indexOf("[");
                const end = pm2Json.lastIndexOf("]");
                if (start !== -1 && end !== -1) {
                    pm2Processes = JSON.parse(pm2Json.substring(start, end + 1));
                }
            } catch (e) {
                console.warn("Failed to parse PM2 stats in main loop", e);
            }
